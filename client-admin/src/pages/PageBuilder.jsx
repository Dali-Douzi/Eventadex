...
function formatDateRange(start, end) {
  ...
  const fmtDT   = (s) => { const d = toDate(s); return hasTime(s) ? `${fmtDate(d, false)}, ${fmtTime(d)}` : fmtDate(d); };

  if (!end)   return fmtDT(start) + (hasTime(start) ? `, ${toDate(start).getFullYear()}` : '');
  if (!start) return fmtDT(end)   + (hasTime(end)   ? `, ${toDate(end).getFullYear()}`   : '');
...
function LivePreview({ config }) {
  ...
  const headerBg        = config.primaryColor || '#2563eb';
  const headerTextColor = config.textColor    || '#ffffff';
  ...
  return (
    <>
      ...
      <div className="pb-browser-chrome">
        ...
        <div className="pb-browser-url">yoursite.com/register</div>
      </div>

      <div className="pb-preview-card pb-preview-card--chromed">
        ...
        >
          ...
          <div className="preview-header-info" style={{ textAlign: headerInfoAlign, flex: hasLogo ? 1 : 'unset', width: hasLogo ? undefined : '100%' }}>
            {config.headerText
              ...
              : <h1 style={{ color: headerTextColor, opacity: 0.4 }}>Event Name</h1>
            }
            {formatDateRange(config.durationStart, config.durationEnd) && (
              <p style={{ color: headerTextColor, opacity: 0.85 }}>{formatDateRange(config.durationStart, config.durationEnd)}</p>
            )}
        ...
        <div className="preview-page-area" style={bgPageStyle}>
          ...
          {/* Form card */}
          <div className="preview-body" style={cardBgStyle}>
            {visibleFields.length === 0 ? (
              <p className="preview-empty">No visible form fields configured</p>
            ) : (
...
/**
 * Fetches the image as a blob (bypasses canvas CORS taint), draws it at 64×64,
 * and returns the most-frequent saturated, non-white, non-transparent pixel
 * colour as a hex string, or null if the image has no useful colours.
 */
...
export default function PageBuilder() {
  ...
  // newField: null = closed | { label, type, options, optionInput } = form open
  const [newField, setNewField]   = useState(null);
  // Refs no longer needed — ImageUploadZone manages its own internal refs

  // ── Fetch on mount ──────────────────────────────────────
  useEffect(() => {
    api.get('/api/admin/page-config')
      .then(({ data }) => {
        setConfig({
  ...
  // ── Upload helpers (accept File object directly) ────────
  async function handleLogoUpload(file) {
    const fd = new FormData(); fd.append('logo', file);
    setLogoLoading(true);
    try { const { data } = await api.post('/api/admin/page-config/logo', fd); set('logoUrl', data.logoUrl); }
    catch (err) { toast(err.response?.data?.message || 'Logo upload failed', 'error'); }
  ...
  async function handleHeaderBannerUpload(file) {
    ...
    try { const { data } = await api.post('/api/admin/page-config/banner/header', fd); set('headerImageUrl', data.headerImageUrl); }
    catch (err) { toast(err.response?.data?.message || 'Header banner upload failed', 'error'); }
    finally { setHeaderBannerLoading(false); }
  }

  async function handleFooterBannerUpload(file) {
    const fd = new FormData(); fd.append('image', file);
    setFooterBannerLoading(true);
    try { const { data } = await api.post('/api/admin/page-config/banner/footer', fd); set('footerImageUrl', data.footerImageUrl); }
    catch (err) { toast(err.response?.data?.message || 'Footer banner upload failed', 'error'); }
    finally { setFooterBannerLoading(false); }
  }

  async function handleBgImageUpload(file) {
    const fd = new FormData(); fd.append('image', file);
    setBgImageLoading(true);
    try { const { data } = await api.post('/api/admin/page-config/banner/bg', fd); set('bodyBgImageUrl', data.bodyBgImageUrl); }
    catch (err) { toast(err.response?.data?.message || 'Background image upload failed', 'error'); }
    finally { setBgImageLoading(false); }
  }
  ...
  function confirmNewField() {
    ...
    const opts = (newField.type === 'select' || newField.type === 'radio')
  ...
  function updateFieldLabel(i, label) {
    set('formFields', config.formFields.map((f, idx) =>
      idx === i ? { ...f, label } : f
    ));
  }
  ...
  async function handleSave() {
    setSaveState('saving');
    try {
      await api.put('/api/admin/page-config', config);
      setSaveState('saved');
    ...
    return (
      <div className="pb-wrapper">
        <div style={{ width: '40%', minWidth: 300, background: 'white', borderRight: '1px solid var(--border)', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="skeleton sk-title" style={{ width: '50%' }} />
          {[...Array(6)].map((_, i) => (
      ...
      <div className="pb-config">
        ...
        <div className="pb-config-header">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <span className="pb-config-title">Page Builder</span>
        ...
        <div className="pb-config-scroll">
          ...
          >
            ...
            <div className="form-group">
              ...
              <ImageUploadZone
                ...
                label="Header Banner"
                hint="PNG, JPG · ideal: 1200 × 300 px"
                objectFit="cover"
              />
              {config.headerImageUrl && (
                <>
            ...
            <div className="form-group">
              ...
              <ImageUploadZone
                ...
                label="Footer Banner"
                hint="PNG, JPG · ideal: 1200 × 200 px"
                objectFit="cover"
              />
              {config.footerImageUrl && (
                <>
            ...
            <div className="form-group">
              ...
              <div className="color-presets">
                {COLOR_PRESETS.map((c) => (
                  <button
                    key={c}
                    className={`color-preset-dot${(config.primaryColor || '#2563eb') === c ? ' selected' : ''}`}
                    style={{ background: c }}
                    title={c}
            ...
            <div className="form-group">
              ...
              <div className="color-presets">
                {['#ffffff', '#1e293b', '#f8fafc', '#fef9c3', '#dbeafe', '#f0fdf4', '#fdf4ff', '#fff7ed'].map((c) => (
                  <button
                    key={c}
                    className={`color-preset-dot${(config.textColor || '#ffffff') === c ? ' selected' : ''}`}
                    style={{ background: c }}
                    title={c}
          ...
          >
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="e.g. Tunis Conference Center, Tunisia"
              />
            </div>
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                placeholder="e.g. © 2026 Acme Events · All rights reserved"
                rows={2}
          ...
          >
              ...
              return (
                <div key={field.fieldName || i} className={`field-row${isDefault ? ' field-row-default' : ''}`}>
                  <div className="field-label-cell">
                    ...
                    />
                    {isDefault && (
                      <span className="field-default-dot" title={isLocked ? 'Required field — always visible and required' : 'Default field — cannot be removed'}>●</span>
                    )}
                  </div>
                  ...
                  <div className="field-reorder">
                    ...
                    <button className="btn-icon-sq" disabled={i === config.formFields.length - 1} onClick={() => moveField(i, 1)} title="Move down">
                    ...
                    {!isDefault && (
                      <button className="btn-icon-sq btn-danger-ghost" onClick={() => removeField(i)} title="Remove field">
              ...
              <div className="optional-palette">
                ...
                <div className="optional-palette-chips">
                  {OPTIONAL_FIELDS.filter((o) => !config.formFields.find((f) => f.fieldName === o.fieldName)).map((o) => (
                    <button key={o.fieldName} className="palette-chip" onClick={() => addOptionalField(o)}>
                      + {o.label}
                    </button>
                  ))}
                </div>
            ...
            {newField ? (
              ...
              <button className="add-link-btn" style={{ marginTop: 8 }} onClick={() => setNewField({ label: '', type: 'text', options: [], optionInput: '' })}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="14" height="14">
                  <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
                </svg>
                Add Custom Field
