...
const VARIABLES = [
  ...
  { token: '{{countdown}}',      desc: 'Text countdown (e.g. "3 days, 2 hours")' },
  { token: '{{countdownImage}}', desc: 'Live countdown image (updates on open)' },
];

// ─── Preview variable substitution ────────────────────────────────────────────
const SAMPLE_COUNTDOWN = '3 days, 14 hours, and 22 minutes';
const COUNTDOWN_PLACEHOLDER = `
  <div style="background:#1e293b;border-radius:8px;padding:20px 0;text-align:center;margin:16px 0;">
    <div style="color:#94a3b8;font-size:11px;font-weight:600;letter-spacing:2px;margin-bottom:6px;">COUNTDOWN</div>
    <div style="color:#fff;font-size:28px;font-weight:700;">3d  14h  22m</div>
  </div>`;
...
function substituteVars(text, eventName) {
  ...
  return text
    ...
    .replace(/\{\{countdown\}\}/g,      SAMPLE_COUNTDOWN)
    .replace(/\{\{countdownImage\}\}/g, COUNTDOWN_PLACEHOLDER);
}

// ─── Live email preview ───────────────────────────────────────────────────────
function ReminderPreview({ tmpl, images, pageLogoUrl, eventName }) {
  ...
  const hdrImgStyle = {
    display: 'block', width: '100%', height: `${hdrH}px`,
    objectFit: tmpl.headerImageFit === 'fill' ? 'contain' : tmpl.headerImageFit === 'max' ? 'fill' : 'cover',
  };
  const ftrH = tmpl.footerImageHeight ?? 120;
  const ftrImgStyle = {
    display: 'block', width: '100%', height: `${ftrH}px`,
    objectFit: tmpl.footerImageFit === 'fill' ? 'contain' : tmpl.footerImageFit === 'max' ? 'fill' : 'cover',
  };

  const btnColor   = tmpl.buttonColor || '#2563eb';
  const subjectTxt = substituteVars(tmpl.subject, eventName);
  const headerTxt  = substituteVars(tmpl.headerText, eventName);
  const bodyTxt    = substituteVars(tmpl.bodyText, eventName);
  const btnLabel   = tmpl.buttonLabel || 'Add to Calendar';
  const footerTxt  = substituteVars(tmpl.footerText, eventName);
  const placement  = tmpl.logoPlacement || 'left';
  const hdrPad     = tmpl.headerPadding ?? 28;
  const ftrPad     = tmpl.footerImagePadding ?? 0;
  const hasHeader  = !!(tmpl.headerText || logoSrc);
    ...
    if (!logoSrc) {
      return (
        <div className="et-email-header-block" style={bandStyle}>
          {headerTxt
            ? <h2 style={{ margin: 0, color: '#fff' }}>{headerTxt}</h2>
    ...
    if (placement === 'center') {
      return (
        <div className="et-email-header-block" style={{ ...bandStyle, textAlign: 'center' }}>
          <img src={logoSrc} alt="Logo" style={{ ...logoImgStyle, margin: '0 auto', display: 'block', marginBottom: headerTxt ? 16 : 0 }}
            onError={(e) => { e.target.style.display = 'none'; }} />
          {headerTxt && <h2 style={{ margin: '16px 0 0', color: '#fff', textAlign: 'center' }}>{headerTxt}</h2>}
        </div>
      );
    }
    if (placement === 'right') {
      return (
        <div className="et-email-header-block" style={bandStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'space-between' }}>
            <div>{headerTxt && <h2 style={{ margin: 0, color: '#fff' }}>{headerTxt}</h2>}</div>
    ...
    return (
      <div className="et-email-header-block" style={bandStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <img src={logoSrc} alt="Logo" style={logoImgStyle}
            onError={(e) => { e.target.style.display = 'none'; }} />
          {headerTxt && <h2 style={{ margin: 0, color: '#fff' }}>{headerTxt}</h2>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="et-preview-outer">
      <span className="et-preview-label">Live Preview · Reminder Email</span>
      <div className="et-subject-row">
        <strong>Subject:</strong>
        <span>{subjectTxt || <em style={{ color: '#cbd5e1' }}>No subject set</em>}</span>
      </div>
      <div className="et-email-card">
        {headerImageSrc && (
          <div className="et-email-header-image-row">
            <img src={headerImageSrc} alt="Header" style={hdrImgStyle}
              onError={(e) => { e.target.style.display = 'none'; }} />
          </div>
        )}
        {renderHeaderBand()}
        {bodyTxt ? (
          <div className="et-email-body-block"
            dangerouslySetInnerHTML={{ __html: bodyTxt.replace(/\n/g, '<br>') }} />
        ) : (
          <div className="et-email-body-block" style={{ color: '#cbd5e1' }}>
            Body text will appear here… Use <code>{'{{countdown}}'}</code> or <code>{'{{countdownImage}}'}</code>
          </div>
        )}
        {tmpl.buttonLabel && (
          <div className="et-btn-block">
            <button className="et-cta-btn"
              style={{ backgroundColor: btnColor, color: tmpl.buttonTextColor || '#ffffff' }}>
              {btnLabel}
            </button>
          </div>
        )}
        {footerImageSrc && (
          <div className="et-email-footer-image-row" style={{ padding: ftrPad > 0 ? ftrPad : 0 }}>
            <img src={footerImageSrc} alt="Footer" style={ftrImgStyle}
              onError={(e) => { e.target.style.display = 'none'; }} />
          </div>
        )}
        {footerTxt && <div className="et-email-footer">{footerTxt}</div>}
      </div>
    </div>
  );
}

// ─── Full email HTML (for customHtml preview) ─────────────────────────────────
function buildReminderPreviewHtml(tmpl, logoSrc, headerImageSrc, footerImageSrc, eventName) {
  ...
  const hdrFit = tmpl.headerImageFit === 'fill' ? 'contain' : tmpl.headerImageFit === 'max' ? 'fill' : 'cover';
  const ftrFit = tmpl.footerImageFit === 'fill' ? 'contain' : tmpl.footerImageFit === 'max' ? 'fill' : 'cover';
...
<head>
  ...
  <style>* { box-sizing: border-box; } body { margin: 0; padding: 24px 16px; background: #e5e7eb; font-family: sans-serif; }</style>
</head>
<body>
  <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10)">
    ${headerImageSrc ? `<img src="${headerImageSrc}" style="width:100%;height:${hdrH}px;object-fit:${hdrFit};display:block" />` : ''}
    ${headerBand}
    <div style="padding:32px 36px">${body}</div>
    ${tmpl.buttonLabel ? `<div style="padding:0 36px 32px;text-align:center">
    ...
    ${footerImageSrc ? `<div style="${ftrPad > 0 ? `padding:${ftrPad}px` : ''}"><img src="${footerImageSrc}" style="width:100%;height:${ftrH}px;object-fit:${ftrFit};display:block" /></div>` : ''}
...
// ─── Image uploader ───────────────────────────────────────────────────────────
function ImageUploader({ label, currentUrl, uploadType, onUploaded, onRemove }) {
  ...
  async function handleChange(e) {
    ...
    try {
      const { data } = await api.post(`/api/admin/reminder-config/image/${uploadType}`, fd, {
    ...
    <div className="et-image-uploader">
      {currentUrl ? (
        <div className="et-image-thumb-row">
          <img src={assetUrl(currentUrl)} alt={label} className="et-image-thumb"
            onError={(e) => { e.target.style.display = 'none'; }} />
          <div className="et-image-thumb-actions">
            <button className="btn btn-sm btn-outline" onClick={() => inputRef.current?.click()} disabled={uploading}>Replace</button>
            <button className="btn btn-sm btn-outline text-danger" onClick={onRemove}>Remove</button>
          </div>
        </div>
      ) : (
        <div className={`et-upload-drop${uploading ? ' et-upload-drop--loading' : ''}`}
          ...
          <span className="et-upload-hint">PNG, JPG, GIF · max 10 MB</span>
        </div>
      )}
  ...
  return (
    <div className="pb-section">
      <button className="pb-section-toggle" onClick={onToggle}>
        ...
        <svg className={`pb-chevron${isOpen ? ' pb-chevron-open' : ''}`}
...
// ─── Default state ────────────────────────────────────────────────────────────
const DEFAULT_SCHEDULE = { enabled: false, schedule: [{ daysBeforeEvent: 7 }], sendHour: 9 };
const DEFAULT_TMPL = {
  subject:            'Reminder: {{eventName}} is in {{countdown}}!',
  headerText:         '',
  bodyText:           'Hi {{firstName}},\n\nJust a reminder that {{eventName}} is coming up in {{countdown}}.\n\n{{countdownImage}}\n\nWe look forward to seeing you there!',
  buttonLabel:        '',
  buttonColor:        '#2563eb',
...
export default function ReminderConfig() {
  ...
  const [pageLogoUrl, setPageLogoUrl] = useState(null);
  const [eventName,   setEventName]   = useState('');
  const [loading,   setLoading]   = useState(true);
  const [saveState, setSaveState] = useState('idle');
  const [testState, setTestState] = useState('idle');
  const [sections, setSections]   = useState({ schedule: true, content: true, images: false, style: false, vars: false });
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      api.get('/api/admin/reminder-config').catch(() => ({ data: {} })),
      api.get('/api/admin/event').catch(() => ({ data: {} })),
      api.get('/api/admin/page-config').catch(() => ({ data: {} })),
    ]).then(([{ data: rc }, { data: ev }, { data: pc }]) => {
      setSched({
        enabled:  rc.enabled  ?? false,
        schedule: rc.schedule?.length ? rc.schedule : [{ daysBeforeEvent: 7 }],
        sendHour: rc.sendHour ?? 9,
      });
      setTmpl({
        subject:            rc.subject            || DEFAULT_TMPL.subject,
        headerText:         rc.headerText         || '',
        bodyText:           rc.bodyText           || DEFAULT_TMPL.bodyText,
        buttonLabel:        rc.buttonLabel        || '',
        buttonColor:        rc.buttonColor        || '#2563eb',
        buttonTextColor:    rc.buttonTextColor    || '#ffffff',
        footerText:         rc.footerText         || '',
        logoWidth:          rc.logoWidth          ?? null,
        logoHeight:         rc.logoHeight         ?? null,
        logoFit:            rc.logoFit            ?? null,
        logoPlacement:      rc.logoPlacement      || 'left',
        headerPadding:      rc.headerPadding      ?? 28,
        headerImageHeight:  rc.headerImageHeight  ?? 200,
        headerImageFit:     rc.headerImageFit     ?? null,
        footerImageHeight:  rc.footerImageHeight  ?? 120,
        footerImageFit:     rc.footerImageFit     ?? null,
        footerImagePadding: rc.footerImagePadding ?? 0,
        customHtml:         rc.customHtml         || '',
      });
      setImages({
        logoUrl:        rc.logoUrl        || null,
        headerImageUrl: rc.headerImageUrl || null,
        footerImageUrl: rc.footerImageUrl || null,
      });
      setPageLogoUrl(pc.logoUrl || ev._pageLogoUrl || null);
      setEventName(ev.name || '');
    }).finally(() => setLoading(false));
  }, []);

  function set(key, value)     { setTmpl((t) => ({ ...t, [key]: value })); }
  function setSc(key, value)   { setSched((s) => ({ ...s, [key]: value })); }
  ...
  function addInterval() {
    setSched((s) => ({
      ...s,
      schedule: [...s.schedule, { daysBeforeEvent: 1 }],
    }));
  }
  function removeInterval(idx) {
    setSched((s) => ({
      ...s,
      schedule: s.schedule.filter((_, i) => i !== idx),
    }));
  }
  function setInterval(idx, days) {
    setSched((s) => {
      const next = [...s.schedule];
      next[idx] = { daysBeforeEvent: days };
      return { ...s, schedule: next };
    });
  }

  // ── Image helpers ────────────────────────────────────────────────────────────
  function setImage(field, url) { setImages((prev) => ({ ...prev, [field]: url })); }
  async function removeImage(field) {
    try {
      await api.delete(`/api/admin/reminder-config/image/${field.replace('Url', '').replace('Image', '').toLowerCase()}`);
  ...
  async function handleSave() {
    setSaveState('saving');
    try {
      await api.put('/api/admin/reminder-config', { ...sched, ...tmpl });
      setSaveState('saved');
  ...
  async function handleSendTest() {
    ...
    try {
      // Save first so the server sees the latest template
      await api.put('/api/admin/reminder-config', { ...sched, ...tmpl });
      await api.post('/api/admin/reminder-config/test');
      showToast('Test reminder sent to your email!', 'success');
    } catch (err) {
        ...
        <div className="pb-config">
          ...
          <div className="pb-config-header">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <span className="pb-config-title">Reminder Emails</span>
              <span style={{ fontSize: 12, color: 'var(--text-light)' }}>
                Automatically email registrants before the event
              </span>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                title="Save & send a test reminder to your account"
              >
          ...
          <div className="pb-config-scroll">
            ...
            {/* ── 1. Schedule ── */}
            <Section title="Schedule" icon={<ScheduleIcon />} isOpen={sections.schedule} onToggle={() => toggleSection('schedule')}>
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label className="label" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span
                    ...
                    onClick={() => setSc('enabled', !sched.enabled)}
                    style={{
                      display: 'inline-flex', alignItems: 'center', cursor: 'pointer',
                      width: 40, height: 22, borderRadius: 11,
                      background: sched.enabled ? 'var(--primary, #2563eb)' : '#cbd5e1',
                      padding: 3, transition: 'background 0.2s', flexShrink: 0,
                    }}
                  >
                    <span style={{
                      width: 16, height: 16, borderRadius: '50%', background: '#fff',
                      transform: sched.enabled ? 'translateX(18px)' : 'translateX(0)',
                      transition: 'transform 0.2s',
                    }} />
                  </span>
                  <span style={{
                    fontSize: 12, fontWeight: 600,
                    color: sched.enabled ? 'var(--primary, #2563eb)' : '#94a3b8',
                  }}>
              ...
              {/* Days-before entries */}
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label className="label">Send reminders (days before event)</label>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {sched.schedule.map((entry, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <input
                        ...
                        min={1}
                        max={365}
                        value={entry.daysBeforeEvent}
                        onChange={(e) => setInterval(idx, Math.max(1, parseInt(e.target.value) || 1))}
                      />
                      <span style={{ fontSize: 13, color: 'var(--text-light)', flex: 1 }}>
                        day{entry.daysBeforeEvent !== 1 ? 's' : ''} before the event
                      </span>
                ...
                <button
                  ...
                  onClick={addInterval}
                  disabled={sched.schedule.length >= 10}
                >
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <select
                  className="input"
                  value={sched.sendHour}
                  onChange={(e) => setSc('sendHour', parseInt(e.target.value))}
                >
                ...
                <p style={{ fontSize: 11, color: 'var(--text-light)', marginTop: 4 }}>
                  Reminders are sent once per day at this UTC hour. The scheduler checks every 30 minutes.
                </p>
              </div>
            ...
            <Section title="Email Content" icon={<ContentIcon />} isOpen={sections.content} onToggle={() => toggleSection('content')}>
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <input className="input" value={tmpl.subject}
                  onChange={(e) => set('subject', e.target.value)}
                  placeholder="e.g. {{eventName}} is in {{countdown}}!" />
              </div>
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label className="label">Header Text</label>
                <input className="input" value={tmpl.headerText}
                  onChange={(e) => set('headerText', e.target.value)}
                  placeholder="e.g. Don't forget, {{firstName}}!" />
              </div>

              {/* Custom HTML body */}
              <div className="et-html-divider"><span>Body Content</span></div>
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <textarea
                  ...
                  placeholder={'<p>Hi <strong>{{firstName}}</strong>,</p>\n<p>{{eventName}} is in <strong>{{countdown}}</strong>.</p>\n{{countdownImage}}'}
                  rows={8}
                  spellCheck={false}
                />
                <p style={{ fontSize: 11, color: 'var(--text-light)', marginTop: 4, lineHeight: 1.5 }}>
                  {tmpl.customHtml
                    ? 'Custom HTML is active — body text below is ignored. Logo, images & button still apply.'
                    : 'Paste HTML to use as the email body. Leave empty to use plain text. Visual controls always apply.'}
                </p>
              </div>
              ...
              <div className="form-group" style={{ marginBottom: 0, opacity: tmpl.customHtml ? 0.4 : 1, pointerEvents: tmpl.customHtml ? 'none' : 'auto', transition: 'opacity 0.2s' }}>
                <label className="label">
                  Body Text{tmpl.customHtml ? <span style={{ fontWeight: 400, color: 'var(--text-light)' }}> (overridden by HTML)</span> : ''}
                </label>
                <textarea
                  className="input"
                  value={tmpl.bodyText}
                  onChange={(e) => set('bodyText', e.target.value)}
                  placeholder={"Hi {{firstName}},\n\n{{eventName}} is in {{countdown}}.\n\n{{countdownImage}}\n\nSee you there!"}
                  rows={6}
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <textarea className="input" value={tmpl.footerText}
                  onChange={(e) => set('footerText', e.target.value)}
                  placeholder="e.g. © 2026 Acme Events · You're receiving this because you registered."
            ...
            <Section title="Images" icon={<ImagesIcon />} isOpen={sections.images} onToggle={() => toggleSection('images')}>
              ...
              <div className="et-image-field">
                ...
                <p className="et-image-hint">Shown inside the colored header band.</p>
                ...
                {images.logoUrl && (
                  <>
                    <div style={{ marginTop: 8, marginBottom: 4 }}>
                      ...
                      <div className="img-fit-buttons">
                        {['left','center','right'].map(p => (
                          <button key={p} className={`img-fit-btn${tmpl.logoPlacement === p ? ' active' : ''}`}
                    ...
                    <div className="img-resize-row">
                      ...
                      <input type="range" className="img-resize-slider" min={5} max={100} step={5}
                        value={tmpl.logoWidth ?? 25} onChange={(e) => set('logoWidth', Number(e.target.value))} />
                      <button className="img-resize-reset" onClick={() => set('logoWidth', null)}>✕</button>
                    </div>
                    <div className="img-resize-row">
                      <span className="img-resize-label">Height: {tmpl.logoHeight ?? '—'}px</span>
                      <input type="range" className="img-resize-slider" min={20} max={200} step={4}
                        value={tmpl.logoHeight ?? 48} onChange={(e) => set('logoHeight', Number(e.target.value))} />
                      <button className="img-resize-reset" onClick={() => set('logoHeight', null)}>✕</button>
                    </div>
                    <div className="img-fit-buttons">
                      <button className={`img-fit-btn${tmpl.logoFit === 'fill' ? ' active' : ''}`}
                        onClick={() => set('logoFit', tmpl.logoFit === 'fill' ? null : 'fill')}>Fill to container</button>
                      <button className={`img-fit-btn${tmpl.logoFit === 'max' ? ' active' : ''}`}
                        onClick={() => set('logoFit', tmpl.logoFit === 'max' ? null : 'max')}>Maximize</button>
                    </div>
                  </>
                )}
                {(images.logoUrl || tmpl.headerText) && (
                  <div className="img-resize-row" style={{ marginTop: 10 }}>
                    ...
                    <input type="range" className="img-resize-slider" min={0} max={60} step={4}
                      value={tmpl.headerPadding ?? 28} onChange={(e) => set('headerPadding', Number(e.target.value))} />
                  </div>
                )}
              </div>
              ...
              <div className="et-image-field">
                ...
                <p className="et-image-hint">Full-width banner at the top of the email. Ideal: 600 × 200 px.</p>
                <ImageUploader label="Header Image" currentUrl={images.headerImageUrl} uploadType="header"
                ...
                {images.headerImageUrl && (
                  <>
                    <div className="img-resize-row">
                      ...
                      <input type="range" className="img-resize-slider" min={80} max={400} step={10}
                        value={tmpl.headerImageHeight ?? 200} onChange={(e) => set('headerImageHeight', Number(e.target.value))} />
                    </div>
                    <div className="img-fit-buttons">
                      <button className={`img-fit-btn${tmpl.headerImageFit === 'fill' ? ' active' : ''}`}
                        onClick={() => set('headerImageFit', tmpl.headerImageFit === 'fill' ? null : 'fill')}>Fill to container</button>
                      <button className={`img-fit-btn${tmpl.headerImageFit === 'max' ? ' active' : ''}`}
                        onClick={() => set('headerImageFit', tmpl.headerImageFit === 'max' ? null : 'max')}>Maximize</button>
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="et-image-field">
                ...
                <p className="et-image-hint">Full-width image in the footer. Ideal: 600 × 120 px.</p>
                <ImageUploader label="Footer Image" currentUrl={images.footerImageUrl} uploadType="footer"
                ...
                {images.footerImageUrl && (
                  <>
                    <div className="img-resize-row">
                      ...
                      <input type="range" className="img-resize-slider" min={40} max={200} step={4}
                        value={tmpl.footerImageHeight ?? 120} onChange={(e) => set('footerImageHeight', Number(e.target.value))} />
                    </div>
                    <div className="img-resize-row">
                      <span className="img-resize-label">Padding: {tmpl.footerImagePadding ?? 0}px</span>
                      <input type="range" className="img-resize-slider" min={0} max={40} step={4}
                        value={tmpl.footerImagePadding ?? 0} onChange={(e) => set('footerImagePadding', Number(e.target.value))} />
                    </div>
                    <div className="img-fit-buttons">
                      <button className={`img-fit-btn${tmpl.footerImageFit === 'fill' ? ' active' : ''}`}
                        onClick={() => set('footerImageFit', tmpl.footerImageFit === 'fill' ? null : 'fill')}>Fill to container</button>
                      <button className={`img-fit-btn${tmpl.footerImageFit === 'max' ? ' active' : ''}`}
                        onClick={() => set('footerImageFit', tmpl.footerImageFit === 'max' ? null : 'max')}>Maximize</button>
                    </div>
                  </>
                )}
              </div>
            ...
            <Section title="Button Style" icon={<StyleIcon />} isOpen={sections.style} onToggle={() => toggleSection('style')}>
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <input className="input" value={tmpl.buttonLabel}
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <div className="color-row">
                  <input type="color" className="color-swatch"
                    value={tmpl.buttonColor || '#2563eb'}
                    onChange={(e) => set('buttonColor', e.target.value)} />
                  <input className="input" value={tmpl.buttonColor || ''}
                    onChange={(e) => set('buttonColor', e.target.value)} placeholder="#2563eb" maxLength={7} />
                </div>
              </div>
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <div className="color-row">
                  <input type="color" className="color-swatch"
                    value={tmpl.buttonTextColor || '#ffffff'}
                    onChange={(e) => set('buttonTextColor', e.target.value)} />
                  <input className="input" value={tmpl.buttonTextColor || ''}
                    onChange={(e) => set('buttonTextColor', e.target.value)} placeholder="#ffffff" maxLength={7} />
                </div>
              </div>
            </Section>
            ...
            <Section title="Template Variables" icon={<VarsIcon />} isOpen={sections.vars} onToggle={() => toggleSection('vars')}>
              <p style={{ fontSize: 12, color: 'var(--text-light)', marginBottom: 8 }}>
                Use these anywhere in the subject, header, body, or footer.
              </p>
              ...
              <p style={{ fontSize: 11, color: 'var(--text-light)', marginTop: 8, lineHeight: 1.6 }}>
                <strong>{'{{countdownImage}}'}</strong> embeds a server-rendered countdown graphic that
                updates live each time the email is opened.
              </p>
            </Section>
          ...
          {tmpl.customHtml ? (
            <div className="et-preview-outer">
              ...
              <div className="et-html-frame-wrap">
                <iframe
                  srcDoc={buildReminderPreviewHtml(
                    tmpl,
