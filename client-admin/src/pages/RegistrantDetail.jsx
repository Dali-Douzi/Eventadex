...
export default function RegistrantDetail() {
  ...
  const [customFieldDefs, setCustomFieldDefs] = useState([]);   // [{ fieldName, label }]
  const [loading,         setLoading]         = useState(true);
  const [error,           setError]           = useState('');
  const [actionState,     setAction]          = useState('');   // ''|'in'|'out'
  const [resendState,     setResend]          = useState('idle'); // idle|loading|ok|error

  // Fetch registrant (enriched) + page config (for custom field labels)
  useEffect(() => {
    setLoading(true);
    Promise.all([
      api.get(`/api/admin/registrants/${id}`),
  ...
  async function handleCheckIn() {
    setAction('in');
    try {
      const { data } = await api.patch(`/api/admin/registrants/${id}/checkin`);
  ...
  async function handleCheckOut() {
    setAction('out');
    try {
      const { data } = await api.patch(`/api/admin/registrants/${id}/checkout`);
      setRegistrant((prev) => ({ ...prev, ...data }));
  ...
  async function handleResend() {
    setResend('loading');
    try {
      await api.post(`/api/admin/registrants/${id}/resend`);
      setResend('ok');
  ...
  if (error || !registrant) {
    return (
      <div className="rdt-wrapper">
        <button className="rdt-back-btn" onClick={() => navigate('/admin/registrants')}>
          ← Back to Registrants
        </button>
        <div className="rdt-error-box">{error || 'Registrant not found.'}</div>
      </div>
    );
  }
  ...
  // Custom fields from the Mongoose Map (serialised as plain object)
  const customEntries = r.customFields
    ? Object.entries(r.customFields).filter(([, v]) => v && String(v).trim())
    : [];

  // Badge config with fallback
  ...
  return (
    <div className="rdt-wrapper">
      ...
      <div className="rdt-header-card">
        <div className="rdt-avatar">
          {(r.firstName?.[0] || '?').toUpperCase()}
        </div>
        <div className="rdt-header-info">
          <h1 className="rdt-name">{r.firstName} {r.lastName}</h1>
